export default function IdentityInfo({ idManager, onReset }) {
  return (
    <div className="bg-white rounded-lg shadow p-4 mb-6 flex justify-between items-center flex-col sm:flex-row">
      <div>
        <p className="mb-1">
          <span className="font-medium">Your ID:</span> {idManager?.vaultysId?.fingerprint || "Unknown"}
        </p>
        <p>
          <span className="font-medium">Type:</span> {idManager?.isHardware() ? "Hardware/WebAuthn" : "Software"}
        </p>
      </div>
